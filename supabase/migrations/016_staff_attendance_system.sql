...
DROP TABLE IF EXISTS staff_leave_balances CASCADE;
...
CREATE TABLE IF NOT EXISTS staff_attendance (
  ...
  -- Stored date for indexing (auto-set by trigger from clock_in)
  clock_in_date DATE,
...
CREATE TRIGGER trigger_set_clock_in_date
...
CREATE POLICY "Users can manage breaks for their tenant"
  ...
  USING (attendance_id IN (
    SELECT id FROM staff_attendance WHERE user_belongs_to_tenant(tenant_id)
  ));
